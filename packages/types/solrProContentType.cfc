<cfcomponent output="false" extends="farcry.core.packages.types.types" displayname="Solr Pro Content Type" hint="Manages content type index information" bFriendly="false" bObjectBroker="true">
	...
	<cffunction name="lookupFacets" access="public" output="false" returntype="struct" hint="Convert facet data to FarCry object labels and offer custom sorting options.">
		...
		<cfargument name="typename" type="string" required="false" default="" hint="If facet is an array of objectIds, provide the typename for faster lookup (not required)" />
		...
		<cfscript>
			...
			if (arrayLen(arguments.aFacets) > 0) {
				if (arguments.typename != "") {
					// var qContentType = queryExecute("select objectid, #arguments.labelFieldname# from #application.dbowner##arguments.typename#", [], { datasource = application.dsn });
					var qContentType = new query(sql = "select objectid, #arguments.labelFieldname# from #application.dbowner##arguments.typename#", datasource = application.dsn).execute().getResult();
				} else {
					var qContentType = queryNew("objectid, #arguments.labelFieldname#", "varchar, varchar");
				}
				for (var i=1; i <= arrayLen(arguments.aFacets); i++) {
					var bAddRecordData = true;
					if (isValid("uuid", arguments.aFacets[i].data)) {
						if (arguments.typename != "") {
							// var qData = queryExecute("select objectId, #arguments.labelFieldname# from qContentType where objectid = ?", [{value = arguments.aFacets[i].data}], { dbtype = "query" });
							...
							if (qData.recordCount > 0) {
								stObj[arguments.labelFieldname] = qData[arguments.labelFieldname][1];
							} else {
								// If no matching record was found, do not add the record to the return array
								bAddRecordData = false;
								stObj[arguments.labelFieldname] = "";
							}
						} else {
							var stObj = application.fapi.getContentObject(objectId = arguments.aFacets[i].data);
						}						
						if (structKeyExists(stObj, "bDefaultObject") && stObj.bDefaultObject is true) {
							var facetLabel = arguments.aFacets[i].data;
						} else if (stObj[arguments.labelFieldname] != "") {
							var facetLabel = stObj[arguments.labelFieldname];
						}
					} else {
						var facetLabel = arguments.aFacets[i].data;
					}
					if (bAddRecordData is true) {
						arrayAppend(stResults.aFacets, {
							"data": arguments.aFacets[i].data,
							"label": facetLabel,
							"count": arguments.aFacets[i].count
						});
						stResults.countTotalFacets = stResults.countTotalFacets + arguments.aFacets[i].count;
					}
